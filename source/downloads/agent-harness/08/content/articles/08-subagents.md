# 08｜子 Agent 何时才算结束？创建、接纳与资源回收

![子 Agent 的责任归属：独立上下文、进程和工作区分别检查](../diagrams/08.svg)
...
本文使用[固定提交源码](../evidence/versions.json)。2026-09-13 的局部实验包含 Pi 示例原函数配受控的真实 Node 子进程，以及 DeepSeek 原始策略/收尾函数；没有启动真实模型 Agent，没有把示例实验当成四产品整体多 Agent 验收。
...
![Codex：注册身份早于首条消息成功](../diagrams/08-codex.svg)
...
![Pi 示例：killed 标志不证明进程已经退出](../diagrams/08-pi.svg)
...
异常路径揭示了示例调度的限制：池等待 `Promise.all(workers)`，一个 worker reject 会使聚合 promise 先返回错误，并不会等待或取消仍运行的同伴。我们的受控实验让 A 抛错、B 仍阻塞，确认聚合已经 reject 而 B 尚未结束。若使用方把“工具已报错”作为所有子工作已经停下的依据，就会产生所有权误判。[调度实测](../evidence/subagent-results.json)
...
![DeepSeek：结果完成后，还要等待句柄清理](../diagrams/08-deepseek.svg)
...
![Grok：未 promotion 时，先等退出再决定删资源](../diagrams/08-grok.svg)
...
2026-09-13 的[子任务实验](../experiments/subagent-lab.ts)共 7 组：Pi 4 组，DeepSeek 3 组。它们包括一个真实存活子进程的取消反例，PASS 表示反例被成功观察并清理，不表示该取消实现通过可靠性验收。原有双进程取消演示继续保留，但不能用它代替项目相关代码的证据。
...
![创建、接纳、结果、结算与退出分开观察](../diagrams/08-summary.svg)
