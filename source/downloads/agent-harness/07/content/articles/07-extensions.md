# 07｜多个 Hook 如何协作？四种 Harness 的扩展与卸载

![插件与扩展生命周期：卸载应收回注册，并等待资源清理](../diagrams/07.svg)
...
这两组问题分别对应扩展的组合语义和生命周期语义。仅列举支持的 Hook 名称，还不足以判断扩展如何组合、如何卸载。本篇沿[固定源码](../evidence/versions.json)追踪三个对象：传给扩展的输入、扩展返回值如何聚合，以及扩展拥有的资源何时被回收。
...
![Codex：同步 Hook 可以并发，改写按完成序选择](../diagrams/07-codex.svg)
...
![Grok：顺序检查原输入，不是变换流水线](../diagrams/07-grok.svg)
...
![Pi durable HookRegistry：变换真正逐级传递](../diagrams/07-pi.svg)
...
2026-09-13 的局部实验调用原始 Registry，验证了三个场景：输入 1 经 A 加一、B 乘十得到 20，B 实际看见 2；before 异常阻断后续，而 after 异常保留前面 patch 并继续后续；等待 A 时注销 B，B 本轮仍执行一次，下一轮不执行。[实测](../evidence/extensions-results.json)
...
![DeepSeek / Cordis：先登记清理，再异步初始化](../diagrams/07-deepseek.svg)
...
这解决的是“资源在卸载以后才初始化完成”的资源遗漏窗口。仅在 `await setup()` 之后才登记 disposer，会让 owner 在等待期间看不到资源。局部实验用两个受控 Promise 分别阻塞 setup 与 cleanup：释放 setup 前 dispose 不结束；cleanup 已开始但未释放时仍不结束；最后清理一次，owner 才结算。[原始 Cordis 实测](../evidence/extensions-results.json)
...
清理失败也需要区分发生在哪一层。`_unload` 捕获每个 effect 的清理错误并记录日志，其他独立 effect 仍可收尾；这也意味着 owner dispose 正常结算不代表没有清理失败。单个 effect 内没有对每个 disposer 单独隔离异常：局部实验让逆序执行的 B 同步抛错，A 没有被执行。这排除了“框架一定会尝试所有 disposer”的过强理解，插件作者仍需让关键 cleanup 自行保证 finally 或错误聚合。[清理失败实验](../evidence/extensions-results.json)
...
2026-09-13 的[扩展实验](../experiments/extensions-lab.ts)共 8 组通过：Pi 3 组，Cordis 5 组；与原来的监听器卸载、依赖等待和 Pi 轻量 after 失败实验分别保存。没有运行 Codex/Grok Hook 引擎，没有测试任意第三方插件，也没有以函数实验替代原生进程验收。
...
![Hook 的组合规则比 Hook 名称更重要](../diagrams/07-summary.svg)
