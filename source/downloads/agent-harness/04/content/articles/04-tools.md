# 04｜工具并发怎样保持顺序？四种 Harness 的准备、执行与提交

![工具执行与并发：完成顺序可以不同于模型历史顺序](../diagrams/04.svg)
...
本文沿四套[固定源码](../evidence/versions.json)追查准备、启动、结果处理和历史提交，比较实际决定顺序的容器、锁与分支。Pi core 和 DeepSeek scheduler 的局部实验提供受控时序观察；Codex/Grok 的行为分析依据源码及只读未运行的上游测试。
...
![Pi：整批准备屏障之后才并发执行](../diagrams/04-pi.svg)
...
补充实验用同步门让 A 保持未完成，确认即使 C 位于最后，B 仍没有启动。它与仅配置全局 sequential 的旧实验不同，直接检验的是**一项工具元数据对整批的影响**。[实验源码](../experiments/deep-dive-lab.ts)
...
旧实验以 A 等待 B 的同步门保证完成事件 B→A，模型结果消息 A→B。源码进一步解释了原因：完成发生在并发闭包内，历史产生于有序数组遍历。不是某次事件碰巧乱序。[实际记录](../evidence/pi-lab-results.json)
...
![DeepSeek：滚动补位与有序提交是两条线](../diagrams/04-deepseek.svg)
...
新增 `04-ds-rolling-pool-ordered-commit` 直接调用上游 scheduler，观察到 C 在 A 释放前已经启动，而 tool/result 记录仍为 0；最终结果按 A、B、C 提交，最大活动 body 为 2。[结果与完整局部轨迹](../evidence/deep-dive-results.json)、[轨迹数据](../evidence/deep-dive-traces.json)
...
![Codex：等待、执行与结果收集分别推进](../diagrams/04-codex.svg)
...
![Grok：局部路径互斥，不等于业务依赖顺序](../diagrams/04-grok.svg)
...
2026-09-13 的补充测试包含 4 个 Pi 工具场景和 5 个 DeepSeek scheduler 场景，另有 2 个执行边界场景。它们保留门控轨迹，避免依赖“睡 10 ms 应该完成”的偶然时序。[全部新增检查](../evidence/deep-dive-results.json)
...
![并发至少有准备、执行、提交三种顺序](../diagrams/04-summary.svg)
