# 06｜失败之后，Agent 如何继续？四个 Harness 的重试、检查点与恢复策略

![失败之后，Agent 如何继续？四个 Harness 的重试、检查点与恢复策略：历史缺少结果，不代表操作没有发生](../diagrams/06.svg)
...
本篇基于[固定源码版本](../evidence/versions.json)。文中的进程崩溃实验为独立机制实验，确实运行了子进程退出和数据库查询，但没有向这四个产品注入崩溃。源码追踪与产品恢复验收分别陈述。
...
sequenceDiagram
  ...
  participant X as 副作用所有者
  H->>L: 保存调用身份与参数
  H->>X: 执行操作
  X->>X: 提交效果
  X-->>H: 返回确认
  Note over H,L: 故障窗口：外部已完成，本地可能仍是 pending
  H->>L: 保存结果并完成耐久性屏障
  H->>H: 将结果纳入后续上下文
```
...
![Codex：先确定哪段历史仍然存活](../diagrams/06-codex.svg)
...
![Pi durable：允许重放需要旧声明和新声明同时成立](../diagrams/06-pi.svg)
...
![DeepSeek：可见、排队与持久化是不同承诺](../diagrams/06-deepseek.svg)
...
![Grok：恢复要分别检查各个状态域](../diagrams/06-grok.svg)
...
[独立机制实验](../experiments/mechanism_lab.py)让 worker 向 SQLite 提交 operation-1，然后调用 `os._exit(73)`，不写结果文件。父进程验证外部效果表已有记录，结果文件不存在，再用相同逻辑操作身份发起恢复执行。

朴素版本最终出现两条效果；拥有唯一 operation key、在副作用数据库里执行幂等插入的版本仍只有一条。退出码、文件缺失及数据库行数均由实际进程和查询验证。[运行结果](../evidence/mechanism-results.json)
...
![恢复成功必须说明恢复了哪个状态域](../diagrams/06-summary.svg)
